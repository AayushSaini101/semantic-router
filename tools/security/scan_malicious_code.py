...
Scans source trees for obfuscation, exfiltration, credential access, and
binary anomalies. All regex signatures live in scan_sensitive_paths.toml
(base64-encoded) so this file is not flagged by endpoint antivirus tools.
...
def load_signatures() -> tuple[
    ...
    long_b64 = re.compile(
        base64.b64decode(data["long_base64"]["pattern_b64"]).decode("utf-8")
    )
...
def scan_binary_anomalies(root: Path, result: ScanResult):
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        in_build = any(
            ...
            for p in (
                "build", "dist", "target", "release", "debug",
